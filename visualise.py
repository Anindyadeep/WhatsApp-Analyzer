import matplotlib.pyplot as plt
import functions as wpf
import seaborn as sns
import plotly.express as px
from wordcloud import WordCloud, STOPWORDS

PATH1 = r'C:\Users\cosmi\Documents\Projects\Project whatsapp\WhatsApp Chat with 🍄...CHILLZONE...🍄.txt'
PATH2 = r'C:\Users\cosmi\Documents\Projects\Project whatsapp\WhatsApp Chat with Crazy World AGCS.txt'

raw_data = wpf.get_raw_data(PATH1)
stats_data = wpf.get_stats_frame(raw_data)
emoji_stats_data = wpf.get_all_emojies_used(stats_data)
data_to_plot = wpf.get_data_to_plot(raw_data)


########################## Group's total user specific visualisations ##########################

# The plot of the emojies vs the number of the frequencies of the emojies

def plot_emoji_frequency(emoji_stats_data):
    fig = px.bar(emoji_stats_data, x="Emoji", y="Emoji_frequency",
                 title="The emojies vs the frequencies of their usages")
    fig.show()


# The plot of the users vs the number of the messages sent by the users


def plot_user_msg_freq(stats_data):
    fig = px.bar(stats_data, x="User", y="Total_msg", title="The users vs the number of the messages sent")
    fig.show()


# The plot of the users vs the number of the media sent by the users

def plot_user_media_freq(stats_data):
    fig = px.bar(stats_data, x="User", y="Total_media",
                 title="The users used vs the frequencies of the medias sent includes (pics, vids, stickesrs, gifs)")
    fig.show()


# The plot of the users vs the number of the links sent by the users

def plot_user_link_freq(stats_data):
    fig = px.bar(stats_data, x="User", y="Total_link", title="The users used vs the number of the links sent")
    fig.show()


# The plot of the users vs the number of the emojies sent by the user

def plot_user_emoji_freq(stats_data):
    fig = px.bar(stats_data, x="User", y="Total_emojies", title="The users used vs the number of emojies sent")
    fig.show()


######### Particular uers's visualisations of the progressions of the messages w.r.t the day #########

def get_active_times_user(data_to_plot, user):
    user = " " + user
    data = data_to_plot.loc[data_to_plot.User == user]
    hours = data.Hour.value_counts().sort_index().index
    counts = data.Hour.value_counts().sort_index().values
    Time = hours
    Time_words = []

    for time in Time:
        if time == 0:
            t = "12 AM"
        elif time == 1:
            t = "1 AM"
        elif time == 2:
            t = "2 AM"
        elif time == 3:
            t = "3 AM"
        elif time == 4:
            t = "4 AM"
        elif time == 5:
            t = "5 AM"
        elif time == 6:
            t = "6 AM"
        elif time == 7:
            t = "7 AM"
        elif time == 8:
            t = "8 AM"
        elif time == 9:
            t = "9 AM"
        elif time == 10:
            t = "10 AM"
        elif time == 11:
            t = "11 AM"
        elif time == 12:
            t = "12 PM"
        elif time == 13:
            t = "1 PM"
        elif time == 14:
            t = "2 PM"
        elif time == 15:
            t = "3 PM"
        elif time == 16:
            t = "4 PM"
        elif time == 17:
            t = "5 PM"
        elif time == 18:
            t = "6 PM"
        elif time == 19:
            t = "7 PM"
        elif time == 20:
            t = "8 PM"
        elif time == 21:
            t = "9 PM"
        elif time == 22:
            t = "10 PM"
        elif time == 23:
            t = "11 PM"
        Time_words.append(t)

    sns.set(rc={'figure.figsize': (8, 6)})
    g = sns.barplot(counts, hours, orient='h')
    g.set(yticklabels=Time_words)
    Title = "The active hours stats of "+user
    plt.title(Title)
    plt.show()


######## Group's as well as user wise visualisations of the progressions of the messages w.r.t the day's hours ########

def get_active_times(data_to_plot, user):
    if user != 'ALL':
        get_active_times_user(data_to_plot, user)
    else:
        hours = data_to_plot.Hour.value_counts().sort_index().index
        counts = data_to_plot.Hour.value_counts().sort_index().values
        Time = hours
        Time_words = []
        for time in Time:
            if time == 0:
                t = "12 AM"
            elif time == 1:
                t = "1 AM"
            elif time == 2:
                t = "2 AM"
            elif time == 3:
                t = "3 AM"
            elif time == 4:
                t = "4 AM"
            elif time == 5:
                t = "5 AM"
            elif time == 6:
                t = "6 AM"
            elif time == 7:
                t = "7 AM"
            elif time == 8:
                t = "8 AM"
            elif time == 9:
                t = "9 AM"
            elif time == 10:
                t = "10 AM"
            elif time == 11:
                t = "11 AM"
            elif time == 12:
                t = "12 PM"
            elif time == 13:
                t = "1 PM"
            elif time == 14:
                t = "2 PM"
            elif time == 15:
                t = "3 PM"
            elif time == 16:
                t = "4 PM"
            elif time == 17:
                t = "5 PM"
            elif time == 18:
                t = "6 PM"
            elif time == 19:
                t = "7 PM"
            elif time == 20:
                t = "8 PM"
            elif time == 21:
                t = "9 PM"
            elif time == 22:
                t = "10 PM"
            elif time == 23:
                t = "11 PM"
            Time_words.append(t)

        sns.set(rc={'figure.figsize': (8, 6)})
        g = sns.barplot(counts, hours, orient='h')
        g.set(yticklabels=Time_words)
        Title = "The active hours stats of the group"
        plt.title(Title)
        plt.show()


######### Particular uers's visualisations of the progressions of the messages w.r.t the day #########

def get_active_days_user(data_to_plot, user):
    user = " " + user
    data_day_wise = data_to_plot.loc[data_to_plot.User == user]
    days_of_week = data_day_wise.apply(lambda row: row.Date_Time.dayofweek, axis=1)
    days = days_of_week.value_counts().sort_index().index
    count_days = days_of_week.value_counts().sort_index().values

    Day_words = []
    for day in days:
        if day == 0:
            day_word = "Mon"
        elif day == 1:
            day_word = "Tue"
        elif day == 2:
            day_word = "Wed"
        elif day == 3:
            day_word = "Thu"
        elif day == 4:
            day_word = "Fri"
        elif day == 5:
            day_word = "Sat"
        elif day == 6:
            day_word = "Sun"
        Day_words.append(day_word)

    sns.set(rc={'figure.figsize': (8, 6)})
    g = sns.barplot(count_days, days, orient='h')
    g.set(yticklabels=Day_words)
    Title = "The active days stats of "+user
    plt.title(Title)
    plt.show()


######## Group's as well as user wise visualisations of the progressions of the messages w.r.t the day ########

def get_active_days(data_to_plot, user):
    if user != 'ALL':
        get_active_days_user(data_to_plot, user)
    else:
        data_day_wise = data_to_plot
        days_of_week = data_day_wise.apply(lambda row: row.Date_Time.dayofweek, axis=1)
        days = days_of_week.value_counts().sort_index().index
        count_days = days_of_week.value_counts().sort_index().values
        Day_words = []
        for day in days:
            if day == 0:
                day_word = "Mon"
            elif day == 1:
                day_word = "Tue"
            elif day == 2:
                day_word = "Wed"
            elif day == 3:
                day_word = "Thu"
            elif day == 4:
                day_word = "Fri"
            elif day == 5:
                day_word = "Sat"
            elif day == 6:
                day_word = "Sun"
            Day_words.append(day_word)
        sns.set(rc={'figure.figsize': (8, 6)})
        g = sns.barplot(count_days, days, orient='h')
        g.set(yticklabels=Day_words)
        Title = "The active days stats of the group"
        plt.title(Title)
        plt.show()


# The word cloud

def make_word_cloud(raw_data):
    data_cloud = raw_data
    indices_to_remove1 = list(data_cloud.loc[data_cloud.msg_text_only.str.contains('media'), 'msg_text_only'].index)
    indices_to_remove2 = list(data_cloud.loc[data_cloud.msg_text_only.str.contains('deleted'), 'msg_text_only'].index)
    data_cloud = data_cloud.drop(indices_to_remove1)
    data_cloud = data_cloud.drop(indices_to_remove2)

    data = data_cloud
    final_msg = ' '
    stop_words = set(STOPWORDS)

    for msg in data.msg_text_only:
        msg = str(msg)
        tokens = msg.split()

        final_msg += " ".join(tokens) + " "

    wordcloud = WordCloud(width=800, height=800,
                          background_color='white',
                          stopwords=stop_words,
                          min_font_size=10).generate(final_msg)
    plt.figure(figsize=(8, 8))
    plt.imshow(wordcloud)
    plt.axis('off')
    plt.tight_layout(pad=0)
    plt.show()